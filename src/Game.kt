...
class Game(var gridSize: Int) {
    ...
    private fun checkForTicTacToe(): Boolean {
        ...
        gridFilled = true
        (0 until gridSize * gridSize).forEach { i ->
            if (grid[i]!!.empty) {
                gridFilled = false
            }
        }
        ...
        columns.forEach { column ->
            ...
            //set finished to true
            columnWin = true
            (0 until column.size - 1).forEach { i ->
                if (column[i]!!.output() !== column[i + 1]!!.output()) {
                    columnWin = false
                }
                (0 until column.size - 1).forEach { j ->
                    if (column[i]!!.empty) {
                        columnWin = false
                    }
                }
        ...
        diagonals.forEach { diagonal ->
            ...
            //set finished to true
            diagonalWin = true
            (0 until diagonal.size - 1).forEach { i ->
                if (diagonal[i]!!.output() !== diagonal[i + 1]!!.output()) {
                    diagonalWin = false
                }
                (0 until diagonal.size - 1).forEach { j ->
                    if (diagonal[i]!!.empty) {
                        diagonalWin = false
                    }
                }
    ...
    private fun drawMap(): String {
        ...
        (2 until gridSize + 1).forEach { row ->
            ...
            for (column in 1..1) {
                meat =
                    "\t\t" + alphabet.substring(row - 1, row) + " " + grid[gridSize * (row - 1) + (column - 1)]!!.output()
                (column + 1 until gridSize + 1).forEach { i ->
