...
class Player     //constructor.  requires string to set player type
    ...
    private val type: String // whether the player is human or AI
) {
    private var index = 0
    ...
    fun go() {
        ...
        if (type === "AI") {
            ...
            Player.Companion.delay(
                1000,
                TicTacToe.game.gridSize
            ) //take a second to go to make it appear as if computer is thinking
            while (turn) {
            ...
            while (turn) {
                ...
                //validate user input
                if (Player.Companion.valid_input(TicTacToe.user_input)) {
                    if (TicTacToe.user_input.length == 2) {
                        column = TicTacToe.user_input.substring(0, 1).toInt()
                        row = Player.Companion.letterToNumber(TicTacToe.user_input.substring(1, 2))
                    } else {
                        column = TicTacToe.user_input.substring(0, 2).toInt()
                        row = Player.Companion.letterToNumber(TicTacToe.user_input.substring(2, 3))
                    }
    ...
    companion object {
        ...
        private fun valid_input(user_input: String): Boolean {
            ...
            if (user_input.length == 2) {
                output = user_input.substring(0, 1).matches() && user_input.substring(1, 2).matches()
            } else if (user_input.length == 3) {
                output =
                    user_input.substring(0, 2).matches() && user_input.substring(2, 3).matches()
