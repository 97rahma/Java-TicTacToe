...
object TicTacToe {
    ...
    fun main(args: Array<String>) {
        ...
        TicTacToe.user_input = TicTacToe.getInput("\n\tWhich mode would you like to play? (1/2): ")

        //Keep asking for an answer from the user until we get a 1 or a 2
        TicTacToe.gameMode(TicTacToe.user_input!!) //gameMode() is defines below
        ...
        TicTacToe.valid_input = false
        while (!TicTacToe.valid_input) {
            if (TicTacToe.user_input!!.length > 0 && TicTacToe.user_input!!.substring(0, 1)
                    .matches() && minimumGameSize <= TicTacToe.user_input!!.toInt() && TicTacToe.user_input!!.toInt() <= maximumGameSize
            ) {
                TicTacToe.valid_input = true
            } else {
                TicTacToe.user_input =
        ...
        if (TicTacToe.user_input!!.toInt() > 15) {
        ...
        gameSize = TicTacToe.user_input!!.toInt()

        //Create a new Game instance
        TicTacToe.game = Game(gameSize)
        ...
        while (!TicTacToe.game!!.finished) {
            for (player in players) {
                player!!.go()
                println(
                    """
                        
                        ${TicTacToe.game!!.output()}
                        """.trimIndent()
                )
                TicTacToe.count += 1
    ...
    private fun gameMode(user_input: String) {
        ...
        TicTacToe.valid_input = false
        while (!TicTacToe.valid_input) {
            if (userInput.length == 1 && userInput.substring(0, 1).matches()) {
                TicTacToe.valid_input = true
            } else {
                userInput = TicTacToe.getInput("\n\tYou must enter '1' or '2' for the game mode: ")
            }
        }
        //Set user input to gameMode for use later
        TicTacToe.gameMode = userInput.toInt()
    }
}